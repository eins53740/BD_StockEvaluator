...
def test_task_1_requirements():
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        print(
            f"✓ REQUIREMENT 2 - Direct colors, no CSS vars: {'PASS' if req2_passed else 'FAIL'}"
        )
        if not req2_passed:
        ...
        readable_count = sum(
            1 for element in readable_elements if element in flowchart_def
        )

        req3_passed = len(problematic_found) == 0 and readable_count >= 4
        print(
            f"✓ REQUIREMENT 3 - Simplified but readable text: {'PASS' if req3_passed else 'FAIL'}"
        )
        if not req3_passed:
        ...
        print(
            f"✓ REQUIREMENT 4 - Valid Mermaid syntax: {'PASS' if req4_passed else 'FAIL'}"
        )
        if not req4_passed:
    ...
    if all_requirements_met:
        ...
        print(
            "✓ Node text simplified to prevent parsing errors while maintaining readability"
        )
    ...
    ), "Task 1 requirements validation failed; see log for details."
