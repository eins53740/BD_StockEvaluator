...
def test_moat_scorecard_combines_manual_and_ai_scores():
    ...
    assessment = MoatAssessmentInput(
        ...
        ai_summaries={
            "switching_costs": {
                "score": 0.92,
                "summary": "High switching costs across enterprise clients.",
            },
            "network_effects": {
                "score": 0.85,
                "summary": "Expanding partner ecosystem with strong retention.",
            },
            "intangibles": {
                "score": 0.80,
                "summary": "Brand strength improving, patents renewed.",
            },
            "cost_advantage": {
                "score": 0.88,
                "summary": "Scale-driven procurement advantages maintained.",
            },
            "efficient_scale": {
                "score": 0.78,
                "summary": "Regional dominance discourages new entrants.",
            },
        },
        qualitative_notes={
            "cost_advantage": ["Management emphasises margin resilience."]
        },
    )

    scorecard = builder.build(assessment)

    assert scorecard.overall_score == pytest.approx(82.08, abs=0.2)
    assert scorecard.moat_rating == "Wide Moat"
    assert scorecard.dimensions["switching_costs"]["combined_score"] == pytest.approx(
        90.8, abs=0.1
    )
    assert (
    ...
    assert (
        "Management emphasises margin resilience."
        in scorecard.dimensions["cost_advantage"]["notes"]
    )
...
def test_moat_scorecard_handles_missing_inputs():
    ...
    assessment = MoatAssessmentInput(
        ...
        ai_summaries={
            "switching_costs": {
                "score": 0.68,
                "summary": "Contract terms provide moderate stickiness.",
            },
            "network_effects": {"score": 0.70, "summary": "User community plateauing."},
            "intangibles": {
                "score": 0.65,
                "summary": "Brand awareness flat year over year.",
            },
            "cost_advantage": {
                "score": 0.55,
                "summary": "Logistics costs rising due to fuel expenses.",
            },
            "efficient_scale": {
                "score": 0.60,
                "summary": "Regional competitors gaining share.",
            },
        },
        qualitative_notes={},
    )
    ...
    assert scorecard.overall_score == pytest.approx(60.1, abs=0.2)
    assert scorecard.moat_rating == "Narrow Moat"
    assert scorecard.dimensions["switching_costs"]["combined_score"] == pytest.approx(
        68.0, abs=0.1
    )
    assert scorecard.dimensions["cost_advantage"]["combined_score"] == pytest.approx(
        55.0, abs=0.1
    )
...
def test_management_quality_analyzer_scores_high_quality():
    ...
    assert result["score"] == pytest.approx(83.4, abs=0.5)
    assert result["rating"] == "High"
    assert any(
        "capital allocation" in highlight.lower() for highlight in result["highlights"]
    )
