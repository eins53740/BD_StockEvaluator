...
class StockEvaluator:
    ...
    def evaluate(self):
        ...
        # 1. Revenue Growth
        rev_status = self._check('rev_growth', 'Revenue Growth (TTM)', operator.ge, 'rev_growth')
        if rev_status == 'PASS':
        ...
        self.path.append(('P/E Ratio', pe_value, f"< {pe_threshold}", 'PASS' if pe_ok else 'FAIL'))
        ...
        if pe_ok:
            ...
            peg_value = (pe_value / rev_growth_pct) if rev_growth_pct and pe_value is not None else None
            peg_threshold = self.thresholds['peg']
            peg_ok = peg_value is not None and 0 < peg_value < peg_threshold
            self.path.append(('PEG Ratio', peg_value, f"< {peg_threshold}", 'PASS' if peg_ok else 'FAIL'))
            if peg_ok:
        ...
        # 4. Net Profit Margin
        margin_status = self._check('margin', 'Net Profit Margin', operator.ge, 'margin')
