...
class StockAnalysisFeatures:
    ...
    def __init__(self, ticker_symbol: str, stock_info: Dict, historical_metrics: Optional[List[Dict]] = None):
    ...
    def get_risk_assessment(self) -> Dict:
        ...
        overall_risk = sum(risk_factors[factor] * weights[factor] 
                          for factor in risk_factors if risk_factors[factor] is not None)
    ...
    def get_dividend_analysis(self) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                'dividend_sustainability': self._assess_dividend_sustainability(payout_ratio),
                'dividend_growth': self._analyze_dividend_growth(dividends) if not dividends.empty else None,
                'yield_attractiveness': self._assess_yield_attractiveness(dividend_yield)
            }
    ...
    def _assess_sector_risk(self) -> Optional[float]:
        ...
        try:
            ...
            moderate_risk_sectors = ['consumer discretionary', 'industrials', 'materials']
    ...
    def _generate_risk_recommendations(self, risk_factors: Dict) -> List[str]:
        ...
        if exceeds(risk_factors.get('volatility_risk'), 70):
            recommendations.append("High volatility detected - consider position sizing carefully")
        
        if exceeds(risk_factors.get('liquidity_risk'), 60):
            recommendations.append("Low liquidity - may be difficult to exit position quickly")
        
        if exceeds(risk_factors.get('financial_risk'), 60):
            recommendations.append("Financial health concerns - monitor debt levels and profitability")
        
        if not recommendations:
            recommendations.append("Risk profile appears manageable for diversified portfolio")
    ...
    def _assess_trend_consistency(self, trends: Dict) -> str:
        ...
        directions = [trends[p].get('trend_direction') for p in trends if 'trend_direction' in trends[p]]
