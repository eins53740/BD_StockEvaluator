...
from .core.service import generate_flowchart_definition
...
def demo_enhanced_features():
    ...
    for stock_name, stock_data in demo_stocks.items():
        ...
        if "error" not in comp_analysis:
            ...
            print(
                f"  Profitability vs Peers: {comp_analysis['profitability_vs_peers']}"
            )
        ...
        if "error" not in div_analysis:
            if div_analysis["current_yield"]:
                ...
                if div_analysis["payout_ratio"]:
                    ...
                    print(
                        f"  Sustainability: {div_analysis['dividend_sustainability']}"
                    )
            else:
...
def demo_flowchart_features():
    ...
    for scenario in scenarios:
        ...
        print(
            f"  ✅ Passed: {pass_count}, ❌ Failed: {fail_count}, ⚠️ Close: {close_count}"
        )
...
def demo_mobile_features():
    ...
    mobile_files = ["static/manifest.json", "static/sw.js", "config.xml"]
